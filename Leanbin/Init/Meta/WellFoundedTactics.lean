...
theorem Nat.lt_add_rightₓ (a b c : Nat) : a < b → a < b+c :=
  fun h => lt_of_lt_of_leₓ h (Nat.le_add_rightₓ _ _)

theorem Nat.lt_add_left (a b c : Nat) : a < b → a < c+b :=
  fun h => lt_of_lt_of_leₓ h (Nat.le_add_leftₓ _ _)
...
unsafe def default_rel_tac (e : expr) (eqns : List expr) : tactic Unit :=
  do 
    let tgt ← target 
    ...
    exact$
        ...
        | expr.local_const _ (Name.mk_string "_mutual" _) _ _,
          expr.app (e@(quote.1 (@hasWellFoundedOfHasSizeof _))) sz => e (mk_alt_sizeof sz)
        | _, _ => rel
...
unsafe def check_target_is_value_lt : tactic Unit :=
  do 
    let quote.1 ((%%ₓlhs) < %%ₓrhs) ← target 
...
unsafe def well_founded_tactics.default : well_founded_tactics :=
  {  }
