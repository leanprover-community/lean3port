...
@[simp]
theorem band_self (b : Bool) : (b && b) = b := by cases b <;> simp

@[simp]
theorem band_tt (b : Bool) : (b && tt) = b := by cases b <;> simp

@[simp]
theorem band_ff (b : Bool) : (b && ff) = ff := by cases b <;> simp

@[simp]
theorem tt_band (b : Bool) : (tt && b) = b := by cases b <;> simp

@[simp]
theorem ff_band (b : Bool) : (ff && b) = ff := by cases b <;> simp

@[simp]
theorem bor_self (b : Bool) : (b || b) = b := by cases b <;> simp

@[simp]
theorem bor_tt (b : Bool) : (b || tt) = tt := by cases b <;> simp

@[simp]
theorem bor_ff (b : Bool) : (b || ff) = b := by cases b <;> simp

@[simp]
theorem tt_bor (b : Bool) : (tt || b) = tt := by cases b <;> simp
...
theorem ff_eq_tt_eq_false : ¬ff = tt := by contradiction

@[simp]
theorem eq_ff_eq_not_eq_tt (b : Bool) : (¬b = tt) = (b = ff) := by cases b <;> simp
...
@[simp]
theorem bor_coe_iff (a b : Bool) : a || b ↔ a ∨ b := by cases a <;> cases b <;> exact by decide
