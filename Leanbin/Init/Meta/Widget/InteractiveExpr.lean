...
| Acc, [] => pure$ (List.map fun lc : local_collection => { lc with locals := lc.locals.reverse })$ List.reverse$ Acc
| Acc, l :: ls =>
  do 
          ...
          lcs.mmap
              fun lc =>
                do 
                  let lh ← local_c lc 
                  let ns ←
                    pure$ lc.locals.map fun n => h "span" [cn "goal-hyp b pr2"] [html.of_name$ expr.local_pp_name n]
  ...
  tc.mk_simple (tactic_view_action γ) filter_type (fun _ => pure$ filter_type.none)
    ...
    fun ⟨⟩ ft =>
      do 
        ...
        set_goals gs 
        let goal_message :=
          if gs.length = 0 then "goals accomplished" else
            if gs.length = 1 then "1 goal" else toString gs.length ++ " goals"
