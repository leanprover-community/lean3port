...
unsafe def rewrite (h : expr) (e : expr) (cfg : rewrite_cfg := {  }) : tactic (expr × expr × List expr) :=
  do 
    let (new_t, prf, metas) ← rewrite_core h e cfg 
    try_apply_opt_auto_param cfg.to_apply_cfg metas 
    return (new_t, prf, metas)

unsafe def rewrite_target (h : expr) (cfg : rewrite_cfg := {  }) : tactic Unit :=
  do 
    let t ← target 
    let (new_t, prf, _) ← rewrite h t cfg 
    replace_target new_t prf

unsafe def rewrite_hyp (h : expr) (hyp : expr) (cfg : rewrite_cfg := {  }) : tactic expr :=
  do 
