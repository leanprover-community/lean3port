...
theorem accessible {z : α} (ac : Acc r z) : Acc (Tc r) z :=
  Acc.recOnₓ ac
    fun x acx ih =>
      Acc.intro x
        fun y rel =>
          Tc.rec_on rel (fun a b rab acx ih => ih a rab) (fun a b c rab rbc ih₁ ih₂ acx ih => Acc.invₓ (ih₂ acx ih) rab)
            acx ih
...
theorem Nat.lt_wf : WellFounded Nat.Lt :=
  ⟨Nat.rec (Acc.intro 0 fun n h => absurd h (Nat.not_lt_zeroₓ n))
      fun n ih =>
        Acc.intro (Nat.succ n)
          fun m h => Or.elim (Nat.eq_or_lt_of_leₓ (Nat.le_of_succ_le_succₓ h)) (fun e => Eq.substr e ih) (Acc.invₓ ih)⟩
...
instance hasWellFoundedOfHasSizeof (α : Sort u) [SizeOf α] : HasWellFounded α :=
  { R := SizeofMeasure α, wf := sizeof_measure_wf α }
...
theorem rprod_sub_lex : ∀ a b, rprod ra rb a b → lex ra rb a b :=
  fun a b h => Prod.Rprod.rec_on h fun a₁ b₁ a₂ b₂ h₁ h₂ => lex.left b₁ b₂ h₁
