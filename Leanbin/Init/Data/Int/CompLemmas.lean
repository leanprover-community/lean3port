...
protected theorem ne_neg_of_ne {a b : ℤ} : a ≠ b → -a ≠ -b :=
  fun h₁ h₂ => absurd (Int.neg_inj h₂) h₁
...
protected theorem bit0_pos {a : ℤ} : 0 < a → 0 < bit0 a :=
  fun h => Int.add_pos h h

protected theorem bit1_pos {a : ℤ} : 0 ≤ a → 0 < bit1 a :=
  fun h => Int.lt_add_of_le_of_pos (Int.add_nonneg h h) Int.zero_lt_one

protected theorem zero_nonneg : 0 ≤ (0 : ℤ) :=
  le_reflₓ 0

protected theorem one_nonneg : 0 ≤ (1 : ℤ) :=
  le_of_ltₓ Int.zero_lt_one

protected theorem bit0_nonneg {a : ℤ} : 0 ≤ a → 0 ≤ bit0 a :=
  fun h => Int.add_nonneg h h

protected theorem bit1_nonneg {a : ℤ} : 0 ≤ a → 0 ≤ bit1 a :=
  fun h => le_of_ltₓ (Int.bit1_pos h)
...
protected theorem nat_abs_bit0_step {a : Int} {n : Nat} (h : nat_abs a = n) : nat_abs (bit0 a) = bit0 n :=
  by 
    rw [←h]
...
protected theorem nat_abs_bit1_nonneg {a : Int} (h : 0 ≤ a) : nat_abs (bit1 a) = bit1 (nat_abs a) :=
  show nat_abs (bit0 a+1) = bit0 (nat_abs a)+nat_abs 1 from
    by 
      rw [Int.nat_abs_add_nonneg (Int.bit0_nonneg h) (le_of_ltₓ Int.zero_lt_one), Int.nat_abs_bit0]
