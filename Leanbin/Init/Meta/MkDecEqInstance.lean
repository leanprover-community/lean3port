...
private unsafe def get_lhs_rhs : tactic (expr × expr) :=
  do 
...
private unsafe def mk_dec_eq_for (lhs : expr) (rhs : expr) : tactic expr :=
  do 
...
private unsafe def apply_eq_of_heq (h : expr) : tactic Unit :=
  do 
...
private unsafe def dec_eq_case_2 (I_name : Name) (F_name : Name) : tactic Unit :=
  do 
    let (lhs, rhs) ← get_lhs_rhs 
...
unsafe def mk_dec_eq_instance_core : tactic Unit :=
  do 
    ...
    if is_recursive env I_name then
        intro1 >>= fun x => induction x (idx_names ++ [v_name, F_name]) (some$ I_name <.> "brec_on") >> return () else
        intro v_name >> return ()
    get_local v_name >>= cases 
    all_goals' (dec_eq_case_1 I_name F_name)

unsafe def mk_dec_eq_instance : tactic Unit :=
  do 
    let env ← get_env 
    let pi x1 i1 d1 (pi x2 i2 d2 b) ← target >>= whnf 
...
unsafe instance binder_info.has_decidable_eq : DecidableEq BinderInfo :=
  by 
    runTac 
      mk_dec_eq_instance
