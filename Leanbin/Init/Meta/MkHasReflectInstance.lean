...
private unsafe def get_has_reflect_type_name : tactic Name :=
  (do
      ...
      return I) <|>
    fail
      "mk_has_reflect_instance tactic failed, target type is expected to be of the form (has_reflect ...)"
...
private unsafe def has_reflect_case (I_name F_name : Name) (field_names : List Name) :
    tactic Unit := do
...
unsafe def mk_has_reflect_instance : tactic Unit := do
  ...
  -- Use brec_on if type is recursive.
      ...
      intro `_v >>= fun x =>
        induction x [v_name, F_name] (some <| .str I_name "brec_on") >> return ()
