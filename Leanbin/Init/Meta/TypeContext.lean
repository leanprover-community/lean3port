...
unsafe instance : Monadₓ type_context :=
  { bind := @type_context.bind, pure := @type_context.pure }

unsafe instance : MonadFail type_context :=
...
unsafe instance type_context_alternative : Alternativeₓ type_context :=
  { failure := fun α => type_context.fail "failed", orelse := fun α x y => type_context.orelse x y }
...
unsafe def print_mvars : type_context Unit :=
  do 
    let mvs ← list_mvars 
    let mvs ←
      pure$
