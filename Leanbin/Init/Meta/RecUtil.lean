...
unsafe def is_type_app_of (e : expr) (I_name : Name) : tactic Bool :=
  do 
    let t ← infer_type e 
...
unsafe def mk_brec_on_rec_value (F_name : Name) (i : Nat) : tactic expr :=
  do 
    let F ← get_local F_name 
    mk_rec_inst_aux F i

unsafe def constructor_num_fields (c : Name) : tactic Nat :=
  do 
    let env ← get_env 
    let decl ← env.get c 
...
unsafe def mk_constructor_arg_names (c : Name) (p : Name) (i : Nat) : tactic (List Name × Nat) :=
  do 
...
unsafe def mk_constructors_arg_names (I : Name) (p : Name) : tactic (List (List Name)) :=
  do 
    let env ← get_env 
...
private unsafe def mk_fresh_arg_name (n : Name) (s : name_set) : tactic (Name × name_set) :=
  do 
...
unsafe def mk_constructor_fresh_names (nparams : Nat) (c : Name) (s : name_set) : tactic (List Name × name_set) :=
  do 
    let d ← get_decl c 
    let t := d.type 
...
unsafe def mk_constructors_fresh_names (I : Name) : tactic (List (List Name)) :=
  do 
    let env ← get_env 
