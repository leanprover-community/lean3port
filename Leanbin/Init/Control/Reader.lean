...
instance : Monadₓ (ReaderTₓ ρ m) :=
  { pure := @ReaderTₓ.pure _ _ _, bind := @ReaderTₓ.bind _ _ _ }
...
@[inline]
protected def adapt {ρ' : Type u} [Monadₓ m] {α : Type u} (f : ρ' → ρ) : ReaderTₓ ρ m α → ReaderTₓ ρ' m α :=
  fun x => ⟨fun r => x.run (f r)⟩
...
instance [Alternativeₓ m] : Alternativeₓ (ReaderTₓ ρ m) :=
  { failure := @ReaderTₓ.failure _ _ _ _, orelse := @ReaderTₓ.orelse _ _ _ _ }

instance ε [Monadₓ m] [MonadExcept ε m] : MonadExcept ε (ReaderTₓ ρ m) :=
  { throw := fun α => ReaderTₓ.lift ∘ throw, catch := fun α x c => ⟨fun r => catch (x.run r) fun e => (c e).run r⟩ }
