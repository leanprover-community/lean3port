...
export InteractionMonad(result result.success result.exception result.cases_on result_to_string mk_exception silent_fail
  orelse' bracket)
...
unsafe def tactic.up.{u₁, u₂} {α : Type u₂} (t : tactic α) : tactic (Ulift.{u₁} α) :=
  fun s =>
    match t s with 
    | success a s' => success (Ulift.up a) s'
    | exception t ref s => exception t ref s

unsafe def tactic.down.{u₁, u₂} {α : Type u₂} (t : tactic (Ulift.{u₁} α)) : tactic α :=
  fun s =>
    match t s with 
...
unsafe instance executor_tactic : executor tactic :=
  { config_type := Unit, Inhabited := ⟨()⟩, execute_with := fun _ => id }
...
-/
unsafe def try_core (t : tactic α) : tactic (Option α) :=
  fun s =>
    match t s with 
...
-/
unsafe def try (t : tactic α) : tactic Unit :=
  fun s =>
    match t s with 
...
succeeds.
-/
unsafe def fail_if_success {α : Type u} (t : tactic α) : tactic Unit :=
  fun s =>
    match t s with 
...
succeeds.
-/
unsafe def success_if_fail {α : Type u} (t : tactic α) : tactic Unit :=
  fun s =>
    match t s with 
...
unsafe def returnopt (e : Option α) : tactic α :=
  fun s =>
    match e with 
    | some a => success a s
...
unsafe def decorate_ex (msg : format) (t : tactic α) : tactic α :=
  fun s =>
    result.cases_on (t s) success
      fun opt_thunk =>
...
@[inline]
unsafe def write (s' : tactic_state) : tactic Unit :=
  fun s => success () s'

/-- Get the tactic_state. -/
@[inline]
unsafe def read : tactic tactic_state :=
  fun s => success s s
...
failure later.
-/
unsafe def capture (t : tactic α) : tactic (tactic_result α) :=
  fun s =>
    match t s with 
...
-/
unsafe def resume {α : Type _} (t : tactic_result α) : tactic α :=
  fun s => t

unsafe def get_options : tactic options :=
  do 
    let s ← read 
    return s.get_options

unsafe def set_options (o : options) : tactic Unit :=
  do 
    let s ← read 
    write (s.set_options o)

unsafe def save_options {α : Type} (t : tactic α) : tactic α :=
  do 
    let o ← get_options 
    let a ← t 
    set_options o 
...
unsafe def returnex {α : Type} (e : exceptional α) : tactic α :=
  fun s =>
    match e with 
...
unsafe def tactic_format_expr (e : expr) : tactic format :=
  do 
    let s ← tactic.read 
...
unsafe def get_env : tactic environment :=
  do 
    let s ← read 
    return$ env s

unsafe def get_decl (n : Name) : tactic declaration :=
  do 
    let s ← read
    (env s).get n
...
unsafe def trace {α : Type u} [has_to_tactic_format α] (a : α) : tactic Unit :=
  do 
    let fmt ← pp a 
    return$ _root_.trace_fmt fmt fun u => ()

unsafe def traceCallStack : tactic Unit :=
  fun state => _root_.trace_call_stack (success () state)

unsafe def timetac {α : Type u} (desc : Stringₓ) (t : Thunkₓ (tactic α)) : tactic α :=
  fun s => timeitₓ desc (t () s)
...
structure apply_cfg where 
  md := semireducible 
  approx := tt 
  NewGoals := new_goals.non_dep_first 
  instances := tt 
  AutoParam := tt 
  optParam := tt 
  unify := tt
...
unsafe axiom apply_core (e : expr) (cfg : apply_cfg := {  }) : tactic (List (Name × expr))
...
unsafe def module_doc_strings : tactic (List (Option Name × Stringₓ)) :=
  do 
    let mod_docs ← olean_doc_strings 
    let mod_docs : List (List (Option Name × Stringₓ)) :=
      mod_docs.filter_map fun d => if d.1.isNone then some (d.2.map fun pos_doc => ⟨none, pos_doc.2⟩) else none 
    let mod_docs := mod_docs.join 
    let e ← get_env 
    let decls :=
      environment.fold e ([] : List Name)
        fun d acc =>
          let n := d.to_name 
...
unsafe def istep {α : Type u} (line0 col0 line col ast : ℕ) (t : tactic α) : tactic Unit :=
  fun s => (@scopeTrace _ line col fun _ => with_ast ast (step t) s).clamp_pos line0 line col

unsafe def is_prop (e : expr) : tactic Bool :=
  do 
    let t ← infer_type e 
    return (t = quote.1 Prop)

/-- Return true iff n is the name of declaration that is a proposition. -/
unsafe def is_prop_decl (n : Name) : tactic Bool :=
  do 
    let env ← get_env 
    let d ← env.get n 
    let t ← return$ d.type 
...
If the target is not a Pi or elet it will try to put it in WHNF.
 -/
unsafe def intro (n : Name) : tactic expr :=
  do 
    let t ← target 
...
unsafe def intros : tactic (List expr) :=
  do 
    let t ← target 
    match t with 
      | expr.pi _ _ _ _ =>
...
unsafe def intros_dep : tactic (List expr) :=
  do 
    let t ← target 
    let proc (b : expr) :=
...
unsafe def resolve_constant (n : Name) : tactic Name :=
  do 
    ...
    match e with 
      ...
      | _ =>
        do 
          ...
          let expr.const n _ ← pure$ e.get_app_fn 
          pure n
...
unsafe def revert_all : tactic Nat :=
  do 
    ...
    match lis with 
      ...
      | some (hi :: his) =>
        revert_lst$ lctx.foldl (fun r h => if h.local_uniq_name = hi.local_uniq_name then [] else h :: r) []
...
unsafe def find_assumption (e : expr) : tactic expr :=
  do 
...
unsafe def save_info (p : Pos) : tactic Unit :=
  do 
    let s ← read 
...
unsafe def assert (h : Name) (t : expr) : tactic expr :=
  do 
    assert_core h t 
    swap 
    let e ← intro h 
...
unsafe def define (h : Name) (t : expr) : tactic expr :=
  do 
    define_core h t 
    swap 
    let e ← intro h 
...
unsafe def num_goals : tactic Nat :=
  do 
    let gs ← get_goals 
    return (length gs)
...
unsafe def rotate_right (n : Nat) [Mod Nat] : tactic Unit :=
  do 
    let ng ← num_goals 
...
unsafe def repeat (t : tactic Unit) : tactic Unit :=
  do 
    let gs ← get_goals 
    repeat_aux t gs []
...
unsafe def solve1 {α} (tac : tactic α) : tactic α :=
  do 
    let gs ← get_goals 
    match gs with 
...
-/
unsafe def focus {α} (ts : List (tactic α)) : tactic (List α) :=
  do 
    let gs ← get_goals 
    focus_aux ts gs []
...
unsafe def focus' (ts : List (tactic Unit)) : tactic Unit :=
  do 
    let gs ← get_goals 
    focus'_aux ts gs []

unsafe def focus1 {α} (tac : tactic α) : tactic α :=
  do 
    let g :: gs ← get_goals 
    match gs with 
      | [] => tac
...
-/
unsafe def all_goals {α} (tac : tactic α) : tactic (List α) :=
  do 
    let gs ← get_goals 
...
unsafe def all_goals' (tac : tactic Unit) : tactic Unit :=
  do 
    let gs ← get_goals 
...
-/
unsafe def any_goals {α} (tac : tactic α) : tactic (List (Option α)) :=
  do 
    let gs ← get_goals 
...
unsafe def any_goals' (tac : tactic Unit) : tactic Unit :=
  do 
    let gs ← get_goals 
...
-/
unsafe def seq {α β} (tac1 : tactic α) (tac2 : α → tactic β) : tactic (List β) :=
  do 
    let g :: gs ← get_goals 
    set_goals [g]
    let a ← tac1 
...
unsafe def seq' (tac1 : tactic Unit) (tac2 : tactic Unit) : tactic Unit :=
  do 
    let g :: gs ← get_goals 
    set_goals [g]
    tac1 
...
-/
unsafe def seq_focus {α β} (tac1 : tactic α) (tacs2 : α → List (tactic β)) : tactic (List β) :=
  do 
    let g :: gs ← get_goals 
    set_goals [g]
    let a ← tac1 
...
one of the produced subgoals (like `focus`).
-/
unsafe def seq_focus' (tac1 : tactic Unit) (tacs2 : List (tactic Unit)) : tactic Unit :=
  do 
    let g :: gs ← get_goals 
    set_goals [g]
    tac1 
...
unsafe def fail_if_no_goals : tactic Unit :=
  do 
    let n ← num_goals 
...
unsafe def try_apply_opt_auto_param (cfg : apply_cfg) (ms : List expr) : tactic Unit :=
  when (cfg.auto_param || cfg.opt_param)$
    mwhen (has_opt_auto_param ms)$
      do 
        let gs ← get_goals 
        ms.mmap'
            fun m =>
              mwhen (bnot <$> is_assigned m)$
                set_goals [m] >> when cfg.opt_param (try apply_opt_param) >> when cfg.auto_param (try apply_auto_param)
        set_goals gs
...
unsafe def try_apply_opt_auto_param_for_apply (cfg : apply_cfg) (ms : List (Name × expr)) : tactic Unit :=
  mwhen (has_opt_auto_param_for_apply ms)$
    do 
      let gs ← get_goals 
      ms.mmap'
          fun m =>
            mwhen (bnot <$> is_assigned m.2)$
              set_goals [m.2] >> when cfg.opt_param (try apply_opt_param) >> when cfg.auto_param (try apply_auto_param)
      set_goals gs

unsafe def apply (e : expr) (cfg : apply_cfg := {  }) : tactic (List (Name × expr)) :=
  do 
...
unsafe def mk_const (c : Name) : tactic expr :=
  do 
    let env ← get_env 
    let decl ← env.get c 
...
unsafe def applyc (c : Name) (cfg : apply_cfg := {  }) : tactic Unit :=
  do 
    let c ← mk_const c 
    apply c cfg 
    skip

unsafe def eapplyc (c : Name) : tactic Unit :=
  do 
    let c ← mk_const c 
    eapply c 
    skip
...
unsafe def mk_mvar : tactic expr :=
  do 
    let u ← mk_meta_univ 
...
unsafe def mk_sorry : tactic expr :=
  do 
    let u ← mk_meta_univ 
    let t ← mk_meta_var (expr.sort u)
...
unsafe def mk_local' (pp_name : Name) (bi : BinderInfo) (type : expr) : tactic expr :=
  do 
...
unsafe def by_contradiction (H : Name) : tactic expr :=
  do 
    let tgt ← target 
...
unsafe def kdependencies (e : expr) (md := reducible) : tactic (List expr) :=
  do 
    let ctx ← local_context 
...
unsafe def refine (e : pexpr) : tactic Unit :=
  do 
    let tgt : expr ← target 
...
-/
unsafe def by_cases (e : expr) (h : Name) : tactic Unit :=
  do 
...
unsafe def new_aux_decl_name : tactic Name :=
  do 
    let env ← get_env 
    let n ← decl_name 
...
unsafe def abstract (tac : tactic Unit) (suffix : Option Name := none) (zeta_reduce := tt) : tactic Unit :=
  do 
    fail_if_no_goals 
    let gs ← get_goals 
...
unsafe def solve_aux {α : Type} (type : expr) (tac : tactic α) : tactic (α × expr) :=
  do 
    let m ← mk_meta_var type 
    let gs ← get_goals 
    set_goals [m]
...
unsafe def in_open_namespaces (d : Name) : tactic Bool :=
  do 
...
unsafe def try_for {α} (max : Nat) (tac : tactic α) : tactic α :=
  fun s =>
...
unsafe def try_for_time {α} (max : Nat) (tac : tactic α) : tactic α :=
  fun s =>
    match _root_.try_for_time max (tac s) with 
    | some r => r
    | none => mk_exception "try_for_time tactic failed, timeout" none s

unsafe def updateex_env (f : environment → exceptional environment) : tactic Unit :=
  do 
    let env ← get_env 
...
unsafe def is_protected_decl (n : Name) : tactic Bool :=
  do 
    let env ← get_env 
...
unsafe def revertible_local_context : tactic (List expr) :=
  do 
    let ctx ← local_context 
...
renamed.
-/
unsafe def rename_many (renames : name_map Name) (strict := tt) (use_unique_names := ff) : tactic Unit :=
  do 
    ...
    let ctx_suffix := ctx.drop_while fun h => (renames.find$ hyp_name h).isNone 
    when strict$
        do 
          let ctx_names := rb_map.set_of_list (ctx_suffix.map hyp_name)
          let invalid_renames := (renames.to_list.map Prod.fst).filter fun h => ¬ctx_names.contains h 
          when ¬invalid_renames.empty$
              fail$
                format.join
...
-/
unsafe def rename_unstable (curr : Name) (new : Name) : tactic Unit :=
  do 
...
unsafe def replace_hyp (h : expr) (new_type : expr) (eq_pr : expr) : tactic expr :=
  do 
...
unsafe def main_goal : tactic expr :=
  do 
    let g :: gs ← get_goals 
    return g

unsafe def with_enable_tags {α : Type} (t : tactic α) (b := tt) : tactic α :=
  do 
...
unsafe def set_main_tag (t : tag) : tactic Unit :=
  do 
    let g ← main_goal 
    set_tag g t
...
unsafe instance : Monadₓ task :=
  { map := @task.map, bind := @task.bind, pure := @task.pure }

namespace Tactic

unsafe def mk_id_proof (prop : expr) (pr : expr) : expr :=
  expr.app (expr.app (expr.const `` id [level.zero]) prop) pr

unsafe def mk_id_eq (lhs : expr) (rhs : expr) (pr : expr) : tactic expr :=
  do 
    let prop ← mk_app `eq [lhs, rhs]
    return$ mk_id_proof prop pr

unsafe def replace_target (new_target : expr) (pr : expr) : tactic Unit :=
  do 
    let t ← target 
    assert `htarget new_target 
    swap 
    let ht ← get_local `htarget 
    let locked_pr ← mk_id_eq t new_target pr 
    mk_eq_mpr locked_pr ht >>= exact

unsafe def eval_pexpr α [reflected α] (e : pexpr) : tactic α :=
  to_expr (pquote.1 (%%ₓe : %%ₓreflect α)) ff ff >>= eval_expr α
