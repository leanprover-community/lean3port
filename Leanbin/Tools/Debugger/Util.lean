...
| c :: cs, r, s =>
  if is_space c then to_qualified_name_core cs r s else
    if c = '.' then if s.is_empty then to_qualified_name_core cs r "" else to_qualified_name_core cs (r <.> s) "" else
      to_qualified_name_core cs r (s.str c)
...
unsafe def show_fn (header : Stringₓ) (fn : Name) (frame : Nat) : vm Unit :=
  do 
    let pos ← pos_info fn 
...
unsafe def show_curr_fn (header : Stringₓ) : vm Unit :=
  do 
    let fn ← vm.curr_fn 
    let sz ← vm.call_stack_size 
    show_fn header fn (sz - 1)

unsafe def is_valid_fn_prefix (p : Name) : vm Bool :=
  do 
    let env ← vm.get_env 
    return$
        env.fold ff
...
unsafe def show_frame (frame_idx : Nat) : vm Unit :=
  do 
    let sz ← vm.call_stack_size 
...
unsafe def show_vars (frame : Nat) : vm Unit :=
  do 
