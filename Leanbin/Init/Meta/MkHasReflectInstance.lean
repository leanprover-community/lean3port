...
private unsafe def mk_has_reflect_instance_for (a : expr) : tactic expr :=
  do 
    let t ← infer_type a 
    do 
...
| I_name, F_name, fname :: fnames, num_rec =>
  do 
...
private unsafe def has_reflect_case (I_name F_name : Name) (field_names : List Name) : tactic Unit :=
  do 
...
unsafe def mk_has_reflect_instance : tactic Unit :=
  do 
    ...
    if is_recursive env I_name then
        intro `_v >>= fun x => induction x [v_name, F_name] (some$ I_name <.> "brec_on") >> return () else
        intro v_name >> return ()
