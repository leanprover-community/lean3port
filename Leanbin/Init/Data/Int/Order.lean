...
protected theorem lt_iff_le_and_ne (a b : ℤ) : a < b ↔ a ≤ b ∧ a ≠ b :=
  Iff.intro (fun h => ⟨le_of_ltₓ h, Int.ne_of_lt h⟩) fun ⟨aleb, aneb⟩ =>
...
protected theorem add_lt_add_left {a b : ℤ} (h : a < b) (c : ℤ) : (c+a) < c+b :=
  Iff.mpr (Int.lt_iff_le_and_ne _ _)
    (And.intro (Int.add_le_add_left (le_of_ltₓ h) _) fun heq =>
      Int.lt_irrefl b
        (by
...
protected theorem add_le_add_three {a b c d e f : ℤ} (h₁ : a ≤ d) (h₂ : b ≤ e) (h₃ : c ≤ f) : ((a+b)+c) ≤ (d+e)+f := by
  apply le_transₓ
...
theorem of_nat_nat_abs_of_nonpos {a : ℤ} (H : a ≤ 0) : (nat_abs a : ℤ) = -a := by
