...
theorem min_le_leftₓ (a b : α) : min a b ≤ a := by
  exact minTac.1 a b

theorem min_le_rightₓ (a b : α) : min a b ≤ b := by
  exact minTac.1 a b

theorem le_minₓ {a b c : α} (h₁ : c ≤ a) (h₂ : c ≤ b) : c ≤ min a b := by
  exact minTac.1 a b

theorem le_max_leftₓ (a b : α) : a ≤ max a b := by
  exact minTac.1 b a

theorem le_max_rightₓ (a b : α) : b ≤ max a b := by
  exact minTac.1 b a

theorem max_leₓ {a b c : α} (h₁ : a ≤ c) (h₂ : b ≤ c) : max a b ≤ c := by
  exact minTac.1 b a
...
@[simp]
theorem min_selfₓ (a : α) : min a a = a := by
  exact minTac.1 a a
...
@[simp]
theorem max_selfₓ (a : α) : max a a = a := by
  exact minTac.1 a a
