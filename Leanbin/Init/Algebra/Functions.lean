...
theorem min_def (a b : α) : min a b = if a ≤ b then a else b :=
  by 
...
theorem min_assocₓ (a b c : α) : min (min a b) c = min a (min b c) :=
  by 
    apply eq_minₓ
    ·
      apply le_transₓ 
...
@[ematch]
theorem min_eq_leftₓ {a b : α} (h : a ≤ b) : min a b = a :=
  by 
    apply Eq.symm 
...
theorem max_assocₓ (a b c : α) : max (max a b) c = max a (max b c) :=
  by 
    apply eq_maxₓ
    ·
      apply le_transₓ 
...
theorem max_eq_leftₓ {a b : α} (h : b ≤ a) : max a b = a :=
  by 
    apply Eq.symm 
