...
theorem ext {x x' : StateTₓ σ m α} (h : ∀ st, x.run st = x'.run st) : x = x' :=
  by 
...
@[simp]
theorem run_bind (f : α → StateTₓ σ m β) : (x >>= f).run st = x.run st >>= fun p => (f p.1).run p.2 :=
  by 
...
theorem run_monad_lift {n} [HasMonadLiftT n m] (x : n α) :
  (monad_lift x : StateTₓ σ m α).run st =
    do 
...
theorem run_adapt {σ' σ''} (st : σ) (split : σ → σ' × σ'') (join : σ' → σ'' → σ) (x : StateTₓ σ' m α) :
  (StateTₓ.adapt split join x : StateTₓ σ m α).run st =
    do 
      let (st, ctx) := split st 
      let (a, st') ← x.run st 
      pure (a, join st' ctx) :=
...
instance (m : Type u → Type v) [Monadₓ m] [IsLawfulMonad m] (σ : Type u) : IsLawfulMonad (StateTₓ σ m) :=
  { id_map :=
      by 
...
theorem ext {x x' : ExceptTₓ ε m α} (h : x.run = x'.run) : x = x' :=
  by 
...
@[simp]
theorem run_map (f : α → β) [IsLawfulMonad m] : (f <$> x).run = Except.mapₓ f <$> x.run :=
  by 
...
instance (m : Type u → Type v) [Monadₓ m] [IsLawfulMonad m] (ε : Type u) : IsLawfulMonad (ExceptTₓ ε m) :=
  { id_map :=
      by 
        intros 
        apply ExceptTₓ.ext 
...
theorem ext {x x' : ReaderTₓ ρ m α} (h : ∀ r, x.run r = x'.run r) : x = x' :=
  by 
    cases x <;> cases x' <;> simp [show x = x' from funext h]
...
@[simp]
theorem run_map (f : α → β) [IsLawfulMonad m] : (f <$> x).run r = f <$> x.run r :=
  by 
...
instance (ρ : Type u) (m : Type u → Type v) [Monadₓ m] [IsLawfulMonad m] : IsLawfulMonad (ReaderTₓ ρ m) :=
  { id_map :=
      by 
...
theorem ext {x x' : OptionTₓ m α} (h : x.run = x'.run) : x = x' :=
  by 
    cases x <;> cases x' <;> simp_all 
...
@[simp]
theorem run_map (f : α → β) [IsLawfulMonad m] : (f <$> x).run = Option.map f <$> x.run :=
  by 
    rw [←bind_pure_comp_eq_map _ x.run]
...
instance (m : Type u → Type v) [Monadₓ m] [IsLawfulMonad m] : IsLawfulMonad (OptionTₓ m) :=
  { id_map :=
      by 
        intros 
        apply OptionTₓ.ext 
