...
unsafe instance : Monadₓ conv :=
  by 
    dunfold conv <;> infer_instance

unsafe instance : MonadFail conv :=
  by 
    dunfold conv <;> infer_instance

unsafe instance : Alternativeₓ conv :=
  by 
    dunfold conv <;> infer_instance
...
unsafe def convert (c : conv Unit) (lhs : expr) (rel : Name := `eq) : tactic (expr × expr) :=
  do 
...
unsafe def lhs : conv expr :=
  do 
    let (_, lhs, rhs) ← target_lhs_rhs 
    return lhs

unsafe def rhs : conv expr :=
  do 
    let (_, lhs, rhs) ← target_lhs_rhs 
    return rhs

/-- `⊢ lhs = rhs` ~~> `⊢ lhs' = rhs` using `h : lhs = lhs'`. -/
unsafe def update_lhs (new_lhs : expr) (h : expr) : conv Unit :=
  do 
    transitivity 
    rhs >>= unify new_lhs 
    exact h 
...
unsafe def change (new_lhs : expr) : conv Unit :=
  do 
...
/-- dsimp the LHS. -/
unsafe def dsimp (s : Option simp_lemmas := none) (u : List Name := []) (cfg : dsimp_config := {  }) : conv Unit :=
  do 
    let s ←
      match s with 
        | some s => return s
