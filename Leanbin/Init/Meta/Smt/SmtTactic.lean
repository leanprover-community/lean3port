...
unsafe instance : Monadₓ smt_tactic :=
  by 
    infer_instance

unsafe instance : Alternativeₓ smt_tactic :=
  by 
    infer_instance

unsafe instance : MonadStateₓ smt_state smt_tactic :=
  by 
    infer_instance
...
unsafe def add_ematch_lemma_core (md : transparency) (as_simp : Bool) (e : expr) : smt_tactic Unit :=
  do 
    let h ← hinst_lemma.mk_core md e as_simp 
    add_lemmas (mk_hinst_singleton h)

unsafe def add_ematch_lemma_from_decl_core (md : transparency) (as_simp : Bool) (n : Name) : smt_tactic Unit :=
  do 
    let h ← hinst_lemma.mk_from_decl_core md n as_simp 
    add_lemmas (mk_hinst_singleton h)

unsafe def add_ematch_eqn_lemmas_for_core (md : transparency) (n : Name) : smt_tactic Unit :=
  do 
...
unsafe def try {α : Type} (t : smt_tactic α) : smt_tactic Unit :=
  ⟨fun ss ts =>
      result.cases_on (t.run ss ts) (fun ⟨a, new_ss⟩ => result.success ((), new_ss))
        fun e ref s' => result.success ((), ss) ts⟩
...
protected unsafe def read : smt_tactic (smt_state × tactic_state) :=
  do 
    let s₁ ← get 
    let s₂ ← tactic.read 
    return (s₁, s₂)

protected unsafe def write : smt_state × tactic_state → smt_tactic Unit :=
  fun ⟨ss, ts⟩ => ⟨fun _ _ => result.success ((), ss) ts⟩
...
unsafe def classical : smt_tactic Bool :=
  do 
    let s ← get 
    return s.classical
...
unsafe def get_goals : smt_tactic (List smt_goal × List expr) :=
  do 
    let (g₁, _) ← smt_tactic.read 
    let g₂ ← tactic.get_goals 
    return (g₁, g₂)

unsafe def set_goals : List smt_goal → List expr → smt_tactic Unit :=
  fun g₁ g₂ => ⟨fun ss => tactic.set_goals g₂ >> return ((), g₁)⟩
...
unsafe def all_goals (tac : smt_tactic Unit) : smt_tactic Unit :=
  do 
    let (ss, ts) ← get_goals 
...
unsafe def seq (tac1 : smt_tactic Unit) (tac2 : smt_tactic Unit) : smt_tactic Unit :=
  do 
...
unsafe def focus1 {α} (tac : smt_tactic α) : smt_tactic α :=
  do 
    let (s :: ss, t :: ts) ← get_goals 
    match ss with 
      | [] => tac
...
unsafe def solve1 (tac : smt_tactic Unit) : smt_tactic Unit :=
  do 
    let (ss, gs) ← get_goals 
    match ss, gs with 
...
unsafe def swap : smt_tactic Unit :=
  do 
    let (ss, ts) ← get_goals 
    match ss, ts with 
...
unsafe def by_cases (e : expr) : smt_tactic Unit :=
  do 
    let c ← classical 
    if c then destruct (expr.app (expr.const `classical.em []) e) else
        do 
...
unsafe def proof_for (e : expr) : smt_tactic expr :=
  do 
    let cc ← to_cc_state 
    cc.proof_for e

/-- Return a refutation for e (i.e., a proof for (not e)), if 'e' has been refuted in the main goal. -/
unsafe def refutation_for (e : expr) : smt_tactic expr :=
  do 
    let cc ← to_cc_state 
    cc.refutation_for e

unsafe def get_facts : smt_tactic (List expr) :=
  do 
    let cc ← to_cc_state 
    return$ cc.eqc_of expr.mk_true

unsafe def get_refuted_facts : smt_tactic (List expr) :=
  do 
    let cc ← to_cc_state 
...
unsafe def using_smt {α} (t : smt_tactic α) (cfg : SmtConfig := {  }) : tactic α :=
  do 
