...
instance decidable_le (a b : ℤ) : Decidable (a ≤ b) :=
  decidable_nonneg _

instance decidable_lt (a b : ℤ) : Decidable (a < b) :=
  decidable_nonneg _
...
attribute [local simp] Int.sub_eq_add_neg Int.add_assoc Int.add_right_neg Int.add_left_neg Int.zero_add Int.add_zero
  Int.neg_add Int.neg_neg Int.neg_zero
...
protected theorem lt_irreflₓ (a : ℤ) : ¬a < a :=
  fun this : a < a =>
    lt.elim this
      fun n =>
        fun hn : (a+Nat.succ n) = a =>
          have  : (a+Nat.succ n) = a+0 :=
            by 
              rw [hn, Int.add_zero]
          have  : Nat.succ n = 0 := Int.coe_nat_inj (Int.add_left_cancel this)
          show False from Nat.succ_ne_zero _ this

protected theorem ne_of_ltₓ {a b : ℤ} (h : a < b) : a ≠ b :=
  fun this : a = b =>
    absurd
      (by 
        rw [this] at h 
...
protected theorem lt_iff_le_and_ne (a b : ℤ) : a < b ↔ a ≤ b ∧ a ≠ b :=
  Iff.intro (fun h => ⟨le_of_ltₓ h, Int.ne_of_lt h⟩)
    fun ⟨aleb, aneb⟩ =>
      le.elim aleb
        fun n =>
          fun hn : (a+n) = b =>
...
protected theorem add_lt_add_left {a b : ℤ} (h : a < b) (c : ℤ) : (c+a) < c+b :=
  Iff.mpr (Int.lt_iff_le_and_ne _ _)
    (And.intro (Int.add_le_add_left (le_of_ltₓ h) _)
      fun heq =>
        Int.lt_irrefl b
          (by 
...
instance : LinearOrderₓ Int :=
  { le := Int.Le, le_refl := Int.le_refl, le_trans := @Int.le_trans, le_antisymm := @Int.le_antisymm, lt := Int.Lt,
    lt_iff_le_not_le := @Int.lt_iff_le_not_le, le_total := Int.le_total, DecidableEq := Int.decidableEq,
    decidableLe := Int.decidableLe, decidableLt := Int.decidableLt }

theorem eq_nat_abs_of_zero_le {a : ℤ} (h : 0 ≤ a) : a = nat_abs a :=
  let ⟨n, e⟩ := eq_coe_of_zero_le h 
  by 
    rw [e] <;> rfl

theorem le_nat_abs {a : ℤ} : a ≤ nat_abs a :=
...
protected theorem eq_neg_of_eq_neg {a b : ℤ} (h : a = -b) : b = -a :=
  by 
    rw [h, Int.neg_neg]

protected theorem neg_add_cancel_left (a b : ℤ) : ((-a)+a+b) = b :=
  by 
    rw [←Int.add_assoc, Int.add_left_neg, Int.zero_add]

protected theorem add_neg_cancel_left (a b : ℤ) : (a+(-a)+b) = b :=
  by 
    rw [←Int.add_assoc, Int.add_right_neg, Int.zero_add]

protected theorem add_neg_cancel_right (a b : ℤ) : ((a+b)+-b) = a :=
  by 
    rw [Int.add_assoc, Int.add_right_neg, Int.add_zero]

protected theorem neg_add_cancel_right (a b : ℤ) : ((a+-b)+b) = a :=
  by 
...
protected theorem sub_eq_zero_of_eq {a b : ℤ} (h : a = b) : a - b = 0 :=
  by 
    rw [h, Int.sub_self]
...
@[simp]
protected theorem neg_eq_of_add_eq_zero {a b : ℤ} (h : (a+b) = 0) : -a = b :=
  by 
...
protected theorem neg_mul_neg (a b : ℤ) : ((-a)*-b) = a*b :=
  by 
    simp 

protected theorem neg_mul_comm (a b : ℤ) : ((-a)*b) = a*-b :=
  by 
    simp 

protected theorem mul_sub (a b c : ℤ) : (a*b - c) = (a*b) - a*c :=
  calc (a*b - c) = (a*b)+a*-c := Int.distrib_left a b (-c)
    _ = (a*b) - a*c :=
    by 
      simp 
...
protected theorem sub_mul (a b c : ℤ) : ((a - b)*c) = (a*c) - b*c :=
  calc ((a - b)*c) = (a*c)+(-b)*c := Int.distrib_right a (-b) c 
    _ = (a*c) - b*c :=
    by 
      simp 
...
protected theorem add_le_add {a b c d : ℤ} (h₁ : a ≤ b) (h₂ : c ≤ d) : (a+c) ≤ b+d :=
  le_transₓ (Int.add_le_add_right h₁ c) (Int.add_le_add_left h₂ b)
...
protected theorem add_lt_add {a b c d : ℤ} (h₁ : a < b) (h₂ : c < d) : (a+c) < b+d :=
  lt_transₓ (Int.add_lt_add_right h₁ c) (Int.add_lt_add_left h₂ b)

protected theorem add_lt_add_of_le_of_lt {a b c d : ℤ} (h₁ : a ≤ b) (h₂ : c < d) : (a+c) < b+d :=
  lt_of_le_of_ltₓ (Int.add_le_add_right h₁ c) (Int.add_lt_add_left h₂ b)

protected theorem add_lt_add_of_lt_of_le {a b c d : ℤ} (h₁ : a < b) (h₂ : c ≤ d) : (a+c) < b+d :=
  lt_of_lt_of_leₓ (Int.add_lt_add_right h₁ c) (Int.add_le_add_left h₂ b)
...
protected theorem add_sub_assoc (a b c : ℤ) : (a+b) - c = a+b - c :=
  by 
...
protected theorem mul_le_mul_of_nonneg_left {a b c : ℤ} (h₁ : a ≤ b) (h₂ : 0 ≤ c) : (c*a) ≤ c*b :=
  by 
    ...
    exact (le_not_le_of_ltₓ (Int.mul_lt_mul_of_pos_left (lt_of_le_not_leₓ h₁ hba) (lt_of_le_not_leₓ h₂ hc0))).left
...
protected theorem mul_le_mul_of_nonneg_right {a b c : ℤ} (h₁ : a ≤ b) (h₂ : 0 ≤ c) : (a*c) ≤ b*c :=
  by 
    ...
    exact (le_not_le_of_ltₓ (Int.mul_lt_mul_of_pos_right (lt_of_le_not_leₓ h₁ hba) (lt_of_le_not_leₓ h₂ hc0))).left

protected theorem mul_le_mul {a b c d : ℤ} (hac : a ≤ c) (hbd : b ≤ d) (nn_b : 0 ≤ b) (nn_c : 0 ≤ c) : (a*b) ≤ c*d :=
  calc (a*b) ≤ c*b := Int.mul_le_mul_of_nonneg_right hac nn_b 
...
protected theorem mul_lt_mul {a b c d : ℤ} (hac : a < c) (hbd : b ≤ d) (pos_b : 0 < b) (nn_c : 0 ≤ c) : (a*b) < c*d :=
  calc (a*b) < c*b := Int.mul_lt_mul_of_pos_right hac pos_b 
    _ ≤ c*d := Int.mul_le_mul_of_nonneg_left hbd nn_c
    

protected theorem mul_lt_mul' {a b c d : ℤ} (h1 : a ≤ c) (h2 : b < d) (h3 : 0 ≤ b) (h4 : 0 < c) : (a*b) < c*d :=
  calc (a*b) ≤ c*b := Int.mul_le_mul_of_nonneg_right h1 h3 
...
protected theorem mul_self_le_mul_self {a b : ℤ} (h1 : 0 ≤ a) (h2 : a ≤ b) : (a*a) ≤ b*b :=
  Int.mul_le_mul h2 h2 h1 (le_transₓ h1 h2)

protected theorem mul_self_lt_mul_self {a b : ℤ} (h1 : 0 ≤ a) (h2 : a < b) : (a*a) < b*b :=
  Int.mul_lt_mul' (le_of_ltₓ h2) h2 h1 (lt_of_le_of_ltₓ h1 h2)
