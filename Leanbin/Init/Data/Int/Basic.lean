...
theorem Int.add_assoc.aux2 (m n k : ℕ) : -[m+1] + -[n+1] + ofNat k = -[m+1] + (-[n+1] + ofNat k) := by
  simp [add_succ]
...
theorem negSucc_mul_subNatNat (m n k : ℕ) : -[m+1] * subNatNat n k = subNatNat (succ m * k) (succ m * n) := by
  have h := Nat.lt_or_ge n k
  cases' h with h h
