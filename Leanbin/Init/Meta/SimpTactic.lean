...
unsafe def revert_and_transform (transform : expr → tactic expr) (h : expr) : tactic Unit :=
  do 
...
unsafe def get_eqn_lemmas_for (deps : Bool) (d : Name) : tactic (List Name) :=
  do 
    let env ← get_env 
...
unsafe def dunfold_target (cs : List Name) (cfg : dunfold_config := {  }) : tactic Unit :=
  do 
    let t ← target 
...
unsafe def delta (cs : List Name) (e : expr) (cfg : delta_config := {  }) : tactic expr :=
  let unfold (u : Unit) (e : expr) : tactic (Unit × expr × Bool) :=
    do 
...
unsafe def delta_target (cs : List Name) (cfg : delta_config := {  }) : tactic Unit :=
  do 
    let t ← target 
...
unsafe def unfold_projs (e : expr) (cfg : unfold_proj_config := {  }) : tactic expr :=
  let unfold (changed : Bool) (e : expr) : tactic (Bool × expr × Bool) :=
    do 
      let new_e ← unfold_proj e cfg.md 
      return (tt, new_e, tt)
  do 
    let (tt, new_e) ← dsimplify_core ff (fun c e => failed) unfold e cfg.to_dsimp_config |
      fail "no projections to unfold"
    return new_e

unsafe def unfold_projs_target (cfg : unfold_proj_config := {  }) : tactic Unit :=
  do 
    let t ← target 
...
unsafe def simp_target (s : simp_lemmas) (to_unfold : List Name := []) (cfg : simp_config := {  })
  (discharger : tactic Unit := failed) : tactic name_set :=
  do 
    let t ← target >>= instantiate_mvars 
...
unsafe def simp_hyp (s : simp_lemmas) (to_unfold : List Name := []) (h : expr) (cfg : simp_config := {  })
  (discharger : tactic Unit := failed) : tactic (expr × name_set) :=
  do 
...
| S, use_ns, ns =>
  do 
    let t ← target 
    if t.is_napp_of `not 1 then intro1_aux use_ns ns >> simp_intros_aux S use_ns ns.tail else
        if t.is_arrow then
          (do 
          ...
          if t.is_pi || t.is_let then intro1_aux use_ns ns >> simp_intros_aux S use_ns ns.tail else
            do 
              ...
              if new_t.is_pi then unsafe_change new_t >> simp_intros_aux S use_ns ns else
...
unsafe def mk_eq_simp_ext (simp_ext : expr → tactic (expr × expr)) : tactic Unit :=
  do 
...
unsafe def join_user_simp_lemmas (no_dflt : Bool) (attrs : List Name) : tactic simp_lemmas :=
  if no_dflt then join_user_simp_lemmas_core simp_lemmas.mk attrs else
    do 
...
unsafe def simp_top_down (pre : expr → tactic (expr × expr)) (cfg : simp_config := {  }) : tactic Unit :=
  do 
    let t ← target 
    let (_, new_target, pr) ←
      simplify_top_down ()
...
unsafe def simp_bottom_up (post : expr → tactic (expr × expr)) (cfg : simp_config := {  }) : tactic Unit :=
  do 
    let t ← target 
    let (_, new_target, pr) ←
      simplify_bottom_up ()
...
unsafe def non_dep_prop_hyps : tactic (List expr) :=
  do 
  ...
  do 
    ...
    if h_type =ₐ new_h_type then
        ...
        do 
          ...
          if new_h_type = quote.1 False then
              do 
                let tgt ← target 
