...
private unsafe def mk_has_sizeof_instance_for (a : expr) (use_default : Bool) : tactic expr :=
  do 
    let t ← infer_type a
    (do 
        ...
        if use_default then return (const `nat.zero []) else
          do 
            let f ← pp t 
            fail
...
| use_default, I_name, F_name, fname :: fnames, num_rec =>
  do 
...
| d, I_name, F_name, ns :: nss =>
  do 
    solve1 (has_sizeof_case d I_name F_name ns)
    for_each_has_sizeof_goal d I_name F_name nss

unsafe def mk_has_sizeof_instance_core (use_default : Bool) : tactic Unit :=
  do 
