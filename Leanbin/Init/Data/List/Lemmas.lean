...
@[simp]
theorem map_append (f : α → β) : ∀ l₁ l₂, map f (l₁ ++ l₂) = map f l₁ ++ map f l₂ :=
  by 
...
@[simp]
theorem map_map (g : β → γ) (f : α → β) (l : List α) : map g (map f l) = map (g ∘ f) l :=
  by 
    induction l <;> simp 
...
@[simp]
theorem nil_bind (f : α → List β) : List.bind [] f = [] :=
  by 
    simp [join, List.bind]

@[simp]
theorem cons_bind x xs (f : α → List β) : List.bind (x :: xs) f = f x ++ List.bind xs f :=
  by 
    simp [join, List.bind]

@[simp]
theorem append_bind xs ys (f : α → List β) : List.bind (xs ++ ys) f = List.bind xs f ++ List.bind ys f :=
  by 
...
@[simp]
theorem mem_append {a : α} {s t : List α} : a ∈ s ++ t ↔ a ∈ s ∨ a ∈ t :=
  by 
...
theorem cons_subset_cons {l₁ l₂ : List α} (a : α) (s : l₁ ⊆ l₂) : a :: l₁ ⊆ a :: l₂ :=
  fun b hin => Or.elim (eq_or_mem_of_mem_cons hin) (fun e : b = a => Or.inl e) fun i : b ∈ l₁ => Or.inr (s i)
...
@[simp]
theorem length_map₂ (f : α → β → γ) l₁ : ∀ l₂, length (map₂ f l₁ l₂) = min (length l₁) (length l₂) :=
  by 
...
| a :: l =>
  by 
    byCases' pa : p a <;> simp [partition, filter, pa, partition_eq_filter_filter l]
...
| a :: l₁, l₂ =>
  by 
    byCases' pa : p a <;> simp [pa, filter_append]
