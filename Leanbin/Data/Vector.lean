...
@[simp]
theorem to_list_cons (a : α) (v : Vector α n) : to_list (cons a v) = a :: to_list v :=
  by 
    cases v 
    rfl
...
@[simp]
theorem to_list_drop {n m : ℕ} (v : Vector α m) : to_list (drop n v) = List.dropₓ n (to_list v) :=
  by 
    cases v 
    rfl

@[simp]
theorem to_list_take {n m : ℕ} (v : Vector α m) : to_list (take n v) = List.takeₓ n (to_list v) :=
  by 
    cases v 
    rfl
