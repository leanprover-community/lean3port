...
protected def pure (a : α) : Parser α :=
  fun input pos => ParseResult.done Pos a
...
protected def fail (msg : Stringₓ) : Parser α :=
  fun _ pos => ParseResult.fail Pos (Dlist.singleton msg)

instance : Monadₓ Parser :=
  { pure := @Parser.pure, bind := @Parser.bind }

instance : IsLawfulMonad Parser :=
  { id_map := @parser.id_map, pure_bind := fun _ _ _ _ => rfl, bind_assoc := @parser.bind_assoc }

instance : MonadFail Parser :=
  { Parser.monad with fail := @Parser.fail }

protected def failure : Parser α :=
  fun _ pos => ParseResult.fail Pos Dlist.empty

protected def orelse (p q : Parser α) : Parser α :=
  fun input pos =>
    match p input Pos with 
    | ParseResult.fail pos₁ expected₁ =>
      if pos₁ ≠ Pos then ParseResult.fail pos₁ expected₁ else
        match q input Pos with 
        | ParseResult.fail pos₂ expected₂ =>
          if pos₁ < pos₂ then ParseResult.fail pos₁ expected₁ else
            if pos₂ < pos₁ then ParseResult.fail pos₂ expected₂ else ParseResult.fail pos₁ (expected₁ ++ expected₂)
        | ok => ok
    | ok => ok

instance : Alternativeₓ Parser :=
...
def decorate_errors (msgs : Thunkₓ (List Stringₓ)) (p : Parser α) : Parser α :=
  fun input pos =>
    match p input Pos with 
...
def sat (p : Charₓ → Prop) [DecidablePred p] : Parser Charₓ :=
  fun input pos =>
    if h : Pos < input.size then
      let c := input.read ⟨Pos, h⟩
...
def remaining : Parser ℕ :=
  fun input pos => ParseResult.done Pos (input.size - Pos)
...
def foldr (f : α → β → β) (p : Parser α) (b : β) : Parser β :=
  fun input pos => foldr_core f p b ((input.size - Pos)+1) input Pos
...
def foldl (f : α → β → α) (a : α) (p : Parser β) : Parser α :=
  fun input pos => foldl_core f a p ((input.size - Pos)+1) input Pos
...
def fix (F : Parser α → Parser α) : Parser α :=
  fun input pos => fix_core F ((input.size - Pos)+1) input Pos
