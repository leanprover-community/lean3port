...
unsafe def save_info (p : Pos) : smt_tactic Unit :=
  do 
...
unsafe instance : interactive.executor smt_tactic :=
  { config_type := SmtConfig, Inhabited := ⟨{  }⟩, execute_with := fun cfg tac => using_smt tac cfg }
...
unsafe def add_fact (q : parse texpr) : smt_tactic Unit :=
  do 
...
unsafe def destruct (q : parse texpr) : smt_tactic Unit :=
  do 
    let p ← tactic.to_expr_strict q 
    smt_tactic.destruct p

unsafe def by_cases (q : parse texpr) : smt_tactic Unit :=
  do 
    let p ← tactic.to_expr_strict q 
...
unsafe def ematch_using (l : parse pexpr_list_or_texpr) : smt_tactic Unit :=
  do 
...
unsafe def rsimp : smt_tactic Unit :=
  do 
    let ccs ← to_cc_state 
...
unsafe def eblast_using (l : parse pexpr_list_or_texpr) : smt_tactic Unit :=
  do 
    let hs ← add_hinst_lemmas_from_pexprs reducible ff l hinst_lemmas.mk 
    smt_tactic.iterate (smt_tactic.ematch_using hs >> smt_tactic.try smt_tactic.close)

unsafe def guard_expr_eq (t : expr) (p : parse$ tk ":=" *> texpr) : smt_tactic Unit :=
  do 
    let e ← to_expr p 
    guardₓ (expr.alpha_eqv t e)

unsafe def guard_target (p : parse texpr) : smt_tactic Unit :=
  do 
    let t ← target 
    guard_expr_eq t p
