...
theorem succ_le_succ {n m : ℕ} : n ≤ m → succ n ≤ succ m :=
  fun h => less_than_or_equal.rec (Nat.le_reflₓ (succ n)) (fun a b => less_than_or_equal.step) h
...
theorem pred_le_pred {n m : ℕ} : n ≤ m → pred n ≤ pred m :=
  fun h =>
    less_than_or_equal.rec_on h (Nat.le_reflₓ (pred n))
      fun n => Nat.rec (fun a b => b) (fun a b c => less_than_or_equal.step) n
...
theorem not_succ_le_self : ∀ n : ℕ, ¬succ n ≤ n :=
  fun n => Nat.rec (not_succ_le_zero 0) (fun a b c => b (le_of_succ_le_succ c)) n
