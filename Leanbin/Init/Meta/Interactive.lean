...
unsafe def propagate_tags (tac : itactic) : tactic Unit :=
  do 
    let tag ← get_main_tag 
    if tag = [] then tac else
        focus1$
          do 
            tac 
            let gs ← get_goals 
...
-/
unsafe def exact (q : parse texpr) : tactic Unit :=
  do 
    let tgt : expr ← target 
...
private unsafe def resolve_name' (n : Name) : tactic expr :=
  do 
    let p ← resolve_name n 
    match p with 
...
-/
unsafe def rewrite (q : parse rw_rules) (l : parse location) (cfg : rewrite_cfg := {  }) : tactic Unit :=
  propagate_tags (rw_core q l cfg)

/--
An abbreviation for `rewrite`.
-/
unsafe def rw (q : parse rw_rules) (l : parse location) (cfg : rewrite_cfg := {  }) : tactic Unit :=
  propagate_tags (rw_core q l cfg)

/--
`rewrite` followed by `assumption`.
-/
unsafe def rwa (q : parse rw_rules) (l : parse location) (cfg : rewrite_cfg := {  }) : tactic Unit :=
...
-/
unsafe def erewrite (q : parse rw_rules) (l : parse location) (cfg : rewrite_cfg := { md := semireducible }) :
  tactic Unit :=
  propagate_tags (rw_core q l cfg)
...
-/
unsafe def erw (q : parse rw_rules) (l : parse location) (cfg : rewrite_cfg := { md := semireducible }) : tactic Unit :=
  propagate_tags (rw_core q l cfg)

/--
Returns the unique names of all hypotheses (local constants) in the context.
-/
private unsafe def hyp_unique_names : tactic name_set :=
  do 
...
-/
private unsafe def hyps_except (hyp_uids : name_set) : tactic (List expr) :=
  do 
    let ctx ← local_context 
...
-/
private unsafe def set_cases_tags (in_tag : tag) (rs : List (Name × List expr)) : tactic Unit :=
  do 
    let gs ← get_goals 
    match gs with 
      | [g] => set_tag g in_tag
            ...
            match hp with 
              | (some h, p) =>
                do 
                  ...
                  get_local x
              | (none, p) => i_to_expr p 
          let e ← if e.is_local_constant then pure e else tactic.generalize e >> intro1 
                  ...
                  nonlocals.mmap$
                      fun arg =>
                        do 
                          ...
                          let h ← intro1 
                          intron n 
                          let locals := arg.fold [] fun e _ acc => if e.is_local_constant then e :: Acc else Acc 
                          locals.mmap' (try ∘ clear)
                          pure h 
                intron (n - 1)
                let e ← intro1 
                pure (e, newvars, locals)
          let to_generalize ← (revert.get_or_else []).mmap tactic.get_local 
...
private unsafe def goal_with_matching_tag (ns : List Name) : tactic (expr × case_tag) :=
  do 
    ...
    match exact_matches, suffix_matches with 
      ...
      | [], [g] => pure g
      | [], _ =>
        let tags : List (List Name) := suffix_matches.map fun ⟨_, t⟩ => t.case_names.reverse 
        fail
...
```
-/
unsafe def case (args : parse case_parser) (tac : itactic) : tactic Unit :=
  do 
    let target_goals ←
      args.mmap
          fun ⟨ns, ids⟩ =>
            do 
              ...
              set_goals [goal]
              match tag with 
                | case_tag.pi _ num_args =>
                  do 
                    intro_lst ids 
                    when (num_ids < num_args)$ intron (num_args - num_ids)
                | case_tag.hyps _ new_hyp_names =>
                  do 
                    let num_new_hyps := new_hyp_names.length 
                    when (num_ids > num_new_hyps)$
                        fail
                          f!
                            "Invalid `case`: You gave {num_ids
                              } names, but the case introduces {num_new_hyps } new hypotheses."
...
unsafe def cases_core (e : expr) (ids : List Name := []) : tactic Unit :=
  do 
    let in_tag ← get_main_tag 
    focus1$
        do 
...
```
-/
unsafe def cases_matching (rec : parse$ (tk "*")?) (ps : parse pexpr_list_or_texpr) : tactic Unit :=
  do 
    let ps ← ps.mmap pexpr_to_pattern 
    if rec.is_none then find_matching_hyp ps >>= cases_core else
...
unsafe def cases_type (one : parse$ (tk "!")?) (rec : parse$ (tk "*")?) (type_names : parse (ident)*) : tactic Unit :=
  do 
    ...
    if rec.is_none then try_cases_for_types type_names (bnot one.is_none) else
        tactic.focus1$ tactic.repeat$ try_cases_for_types type_names (bnot one.is_none)
...
private unsafe def assume_core (n : Name) (ty : pexpr) :=
  do 
    let t ← target 
...
private unsafe def constructor_matching_aux (ps : List pattern) : tactic Unit :=
  do 
    let t ← target 
    ps.mfirst fun p => match_pattern p t 
    constructor

unsafe def constructor_matching (rec : parse$ (tk "*")?) (ps : parse pexpr_list_or_texpr) : tactic Unit :=
  do 
    let ps ← ps.mmap pexpr_to_pattern 
...
-/
unsafe def injection (q : parse texpr) (hs : parse with_ident_list) : tactic Unit :=
  do 
    let e ← i_to_expr q 
...
-/
unsafe def injections (hs : parse with_ident_list) : tactic Unit :=
  do 
...
unsafe structure simp_config_ext extends simp_config where 
...
-/
unsafe def decode_simp_arg_list (hs : List simp_arg_type) : tactic$ List pexpr × List Name × List Name × Bool :=
  do 
    let (hs, ex, all) ←
      hs.mfoldl
          (fun r : List pexpr × List Name × Bool h =>
            do 
              let (es, ex, all) := r 
              match h with 
...
unsafe def decode_simp_arg_list_with_symm (hs : List simp_arg_type) :
  tactic$ List (pexpr × Bool) × List Name × List Name × Bool :=
  do 
    let (hs, ex, all) :=
      hs.foldl
        (fun r h =>
...
unsafe def simp_core_aux (cfg : simp_config) (discharger : tactic Unit) (s : simp_lemmas) (u : List Name)
  (hs : List expr) (tgt : Bool) : tactic name_set :=
  do 
    let (to_remove, lmss) ←
    ...
    let (lms, goal_simplified) ←
      if tgt then (simp_target s u cfg discharger >>= fun ns => return (ns, tt)) <|> return (mk_name_set, ff) else
          return (mk_name_set, ff)
    guardₓ (cfg.fail_if_unchanged = ff ∨ to_remove.length > 0 ∨ goal_simplified) <|>
        fail "simplify tactic failed to simplify"
    to_remove.reverse.mmap' fun h => try (clear h)
    return (lmss.union lms)

unsafe def simp_core (cfg : simp_config) (discharger : tactic Unit) (no_dflt : Bool) (hs : List simp_arg_type)
  (attr_names : List Name) (locat : loc) : tactic name_set :=
  do 
    let lms ←
      match locat with 
        | loc.wildcard =>
          do 
            let (all_hyps, s, u) ← mk_simp_set_core no_dflt attr_names hs tt 
            if all_hyps then tactic.simp_all s u cfg discharger else
                do 
...
unsafe def simp_intros (ids : parse (ident_)*) (no_dflt : parse only_flag) (hs : parse simp_arg_list)
  (attr_names : parse with_ident_list) (cfg : simp_intros_config := {  }) : tactic Unit :=
  do 
    let (s, u) ← mk_simp_set no_dflt attr_names hs 
    when (¬u.empty) (fail s! "simp_intros tactic does not support {u}")
    ...
    match l with 
      | loc.wildcard =>
        do 
          ...
          intron n
      | _ => l.apply (fun h => dsimp_hyp h s u cfg) (dsimp_target s u cfg)
...
private unsafe def to_qualified_name (n : Name) : tactic Name :=
  do 
    let env ← get_env 
    if env.contains n then return n else
        do 
...
-/
unsafe def dunfold (cs : parse (ident)*) (l : parse location) (cfg : dunfold_config := {  }) : tactic Unit :=
  match l with 
  | loc.wildcard =>
    do 
  ...
  | _ =>
    do 
      let new_cs ← to_qualified_names cs 
      l.apply (fun h => dunfold_hyp cs h cfg) (dunfold_target new_cs cfg)
...
| cs, l =>
  do 
    let new_cs ← to_qualified_names cs 
    l.apply (delta_hyp new_cs) (delta_target new_cs)

private unsafe def unfold_projs_hyps (cfg : unfold_proj_config := {  }) (hs : List Name) : tactic Bool :=
  hs.mfoldl
    (fun r h =>
      do 
        let h ← get_local h 
...
-/
unsafe def unfold_projs (l : parse location) (cfg : unfold_proj_config := {  }) : tactic Unit :=
  match l with 
  | loc.wildcard =>
    do 
      let ls ← local_context 
      let b₁ ← unfold_projs_hyps cfg (ls.map expr.local_pp_name)
      let b₂ ← tactic.unfold_projs_target cfg >> return tt <|> return ff 
...
-/
unsafe def unfold (cs : parse (ident)*) (locat : parse location) (cfg : unfold_config := {  }) : tactic Unit :=
  do 
...
-/
unsafe def unfold1 (cs : parse (ident)*) (locat : parse location) (cfg : unfold_config := { singlePass := tt }) :
  tactic Unit :=
  unfold cs locat cfg
...
unsafe def guard_expr_eq (t : expr) (p : parse$ tk ":=" *> texpr) : tactic Unit :=
  do 
    let e ← to_expr p 
...
-/
unsafe def guard_target (p : parse texpr) : tactic Unit :=
  do 
    let t ← target 
    guard_expr_eq t p
...
-/
unsafe def type_check (p : parse texpr) : tactic Unit :=
  do 
    let e ← to_expr p 
    tactic.type_check e 
    infer_type e >>= trace
...
-/
unsafe def show (q : parse texpr) : tactic Unit :=
  do 
    let gs ← get_goals 
    show_aux q gs []
...
unsafe def specialize (p : parse texpr) : tactic Unit :=
  focus1$
    do 
      ...
      if h.is_local_constant then tactic.note h.local_pp_name none e >> try (tactic.clear h) >> rotate 1 else
          tactic.fail "specialize requires a term of the form `h x_1 .. x_n` where `h` appears in the local context"
...
unsafe def has_dup : tactic Bool :=
  do 
    ...
    let p : name_set × Bool :=
      ctx.foldl
        (fun ⟨s, r⟩ h =>
          if r then (s, r) else if s.contains h.local_pp_name then (s, tt) else (s.insert h.local_pp_name, ff))
        (mk_name_set, ff)
    return p.2
...
protected unsafe def apply_inj_lemma : tactic Unit :=
  do 
    let h ← intro `h 
...
theorem Sum.inl.inj_eq {α : Type u} (β : Type v) (a₁ a₂ : α) : (@Sum.inl α β a₁ = Sum.inl a₂) = (a₁ = a₂) :=
  by 
    runTac 
      tactic.mk_inj_eq

theorem Sum.inr.inj_eq (α : Type u) {β : Type v} (b₁ b₂ : β) : (@Sum.inr α β b₁ = Sum.inr b₂) = (b₁ = b₂) :=
  by 
    runTac 
      tactic.mk_inj_eq

theorem Psum.inl.inj_eq {α : Sort u} (β : Sort v) (a₁ a₂ : α) : (@Psum.inl α β a₁ = Psum.inl a₂) = (a₁ = a₂) :=
  by 
    runTac 
      tactic.mk_inj_eq

theorem Psum.inr.inj_eq (α : Sort u) {β : Sort v} (b₁ b₂ : β) : (@Psum.inr α β b₁ = Psum.inr b₂) = (b₁ = b₂) :=
  by 
    runTac 
      tactic.mk_inj_eq

theorem Sigma.mk.inj_eq {α : Type u} {β : α → Type v} (a₁ : α) (b₁ : β a₁) (a₂ : α) (b₂ : β a₂) :
  (Sigma.mk a₁ b₁ = Sigma.mk a₂ b₂) = (a₁ = a₂ ∧ HEq b₁ b₂) :=
  by 
    runTac 
      tactic.mk_inj_eq

theorem Psigma.mk.inj_eq {α : Sort u} {β : α → Sort v} (a₁ : α) (b₁ : β a₁) (a₂ : α) (b₂ : β a₂) :
  (Psigma.mk a₁ b₁ = Psigma.mk a₂ b₂) = (a₁ = a₂ ∧ HEq b₁ b₂) :=
  by 
    runTac 
      tactic.mk_inj_eq

theorem Subtype.mk.inj_eq {α : Sort u} {p : α → Prop} (a₁ : α) (h₁ : p a₁) (a₂ : α) (h₂ : p a₂) :
  (Subtype.mk a₁ h₁ = Subtype.mk a₂ h₂) = (a₁ = a₂) :=
  by 
    runTac 
      tactic.mk_inj_eq

theorem Option.some.inj_eq {α : Type u} (a₁ a₂ : α) : (some a₁ = some a₂) = (a₁ = a₂) :=
  by 
    runTac 
      tactic.mk_inj_eq

theorem List.cons.inj_eq {α : Type u} (h₁ : α) (t₁ : List α) (h₂ : α) (t₂ : List α) :
  (List.cons h₁ t₁ = List.cons h₂ t₂) = (h₁ = h₂ ∧ t₁ = t₂) :=
  by 
    runTac 
      tactic.mk_inj_eq

theorem Nat.succ.inj_eq (n₁ n₂ : Nat) : (Nat.succ n₁ = Nat.succ n₂) = (n₁ = n₂) :=
  by 
    runTac 
      tactic.mk_inj_eq
