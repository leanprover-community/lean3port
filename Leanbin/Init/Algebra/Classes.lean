...
instance (priority := 100) is_symm_op_of_is_symm (α : Type u) (r : α → α → Prop) [IsSymm α r] : IsSymmOp α Prop r :=
...
instance is_total_preorder_is_preorder (α : Type u) (r : α → α → Prop) [s : IsTotalPreorder α r] : IsPreorder α r :=
  { trans := s.trans, refl := fun a => Or.elim (@IsTotal.total _ r _ a a) id id }
...
theorem esymm {a b : α} : a ≈ b → b ≈ a :=
  fun ⟨h₁, h₂⟩ => ⟨h₂, h₁⟩

theorem etrans {a b c : α} : a ≈ b → b ≈ c → a ≈ c :=
  incomp_trans

theorem not_lt_of_equiv {a b : α} : a ≈ b → ¬a≺b :=
  fun h => h.1

theorem not_lt_of_equiv' {a b : α} : a ≈ b → ¬b≺a :=
  fun h => h.2

instance IsEquiv : IsEquiv α equiv :=
...
theorem not_lt_of_lt {α : Type u} {lt : α → α → Prop} [IsStrictOrder α lt] {a b} : lt a b → ¬lt b a :=
