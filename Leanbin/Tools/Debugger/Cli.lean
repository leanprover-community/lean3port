...
unsafe def add_breakpoint (s : State) (args : List Stringₓ) : vm State :=
  ...
  | [arg] =>
    do 
      ...
      if ok then return { s with fnBps := fn :: List.filterₓ (fun fn' => fn ≠ fn') s.fn_bps } else
...
unsafe def down_cmd (frame : Nat) : vm Nat :=
  do 
    ...
    if is_eof then
        ...
        do 
          ...
          match tks with 
            ...
            | cmd :: args =>
              if cmd = "help" ∨ cmd = "h" then show_help >> cmd_loop_core s frame [] else
                if cmd = "exit" then return { s with md := mode.done } else
                  if cmd = "run" ∨ cmd = "r" then return { s with md := mode.run } else
                    if cmd = "step" ∨ cmd = "s" then return { s with md := mode.step } else
                      if cmd = "break" ∨ cmd = "b" then
                        do 
...
unsafe def cmd_loop (s : State) (default_cmd : List Stringₓ) : vm State :=
  do 
...
unsafe def prune_active_bps (s : State) : vm State :=
  do 
    let sz ← vm.call_stack_size 
    return { s with activeBps := prune_active_bps_core sz s.active_bps }

unsafe def updt_csz (s : State) : vm State :=
  do 
    let sz ← vm.call_stack_size 
    return { s with csz := sz }

unsafe def init_transition (s : State) : vm State :=
  do 
    let opts ← vm.get_options 
    if opts.get_bool `server ff then return { s with md := mode.done } else
        do 
          let bps ← vm.get_attribute `breakpoint 
          let new_s ← return { s with fnBps := bps }
          if opts.get_bool `debugger.autorun ff then return { new_s with md := mode.run } else
              do 
...
unsafe def step_transition (s : State) : vm State :=
  do 
    let sz ← vm.call_stack_size 
    if sz = s.csz then return s else
        do 
          show_curr_fn "step"
          cmd_loop s ["s"]
...
unsafe def in_active_bps (s : State) : vm Bool :=
  do 
    let sz ← vm.call_stack_size 
...
unsafe def run_transition (s : State) : vm State :=
  do 
    let b1 ← in_active_bps s 
    let b2 ← bp_reached s 
    if b1 ∨ Not b2 then return s else
        do 
...
unsafe def step_fn (s : State) : vm State :=
  do 
    let s ← prune_active_bps s 
    if s.md = mode.init then
        do 
          let new_s ← init_transition s 
          updt_csz new_s
      else
        if s.md = mode.done then return s else
          if s.md = mode.step then
            do 
              let new_s ← step_transition s 
              updt_csz new_s
          else
            if s.md = mode.run then
              do 
