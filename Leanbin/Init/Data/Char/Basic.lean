...
theorem ne_of_vne {c d : Charₓ} (h : c.val ≠ d.val) : c ≠ d :=
  fun h' => absurd (veq_of_eq h') h

theorem vne_of_ne {c d : Charₓ} (h : c ≠ d) : c.val ≠ d.val :=
  fun h' => absurd (eq_of_veq h') h

end Charₓ

instance : DecidableEq Charₓ :=
  fun i j => decidableOfDecidableOfIff (Nat.decidableEq i.val j.val) ⟨Charₓ.eq_of_veq, Charₓ.veq_of_eq⟩
