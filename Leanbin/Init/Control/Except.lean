...
@[inline]
protected def monad_map {m'} [Monadₓ m'] {α} (f : ∀ {α}, m α → m' α) : ExceptTₓ ε m α → ExceptTₓ ε m' α :=
  fun x => ⟨f x.run⟩

instance m' [Monadₓ m'] : MonadFunctorₓ m m' (ExceptTₓ ε m) (ExceptTₓ ε m') :=
  ⟨@monad_map m' _⟩

instance : Monadₓ (ExceptTₓ ε m) :=
  { pure := @return, bind := @bind }

protected def adapt {ε' α : Type u} (f : ε → ε') : ExceptTₓ ε m α → ExceptTₓ ε' m α :=
  fun x => ⟨Except.mapErrorₓ f <$> x.run⟩
...
instance m ε [Monadₓ m] : MonadExcept ε (ExceptTₓ ε m) :=
  { throw := fun α => ExceptTₓ.mk ∘ pure ∘ Except.error, catch := @ExceptTₓ.catch ε _ _ }
