...
unsafe def save_info (p : Pos) : conv Unit :=
  do 
    let s ← tactic.read 
...
unsafe def dsimp (no_dflt : parse only_flag) (es : parse tactic.simp_arg_list) (attr_names : parse with_ident_list)
  (cfg : Tactic.DsimpConfig := {  }) : conv Unit :=
  do 
...
unsafe def find (p : parse parser.pexpr) (c : itactic) : conv Unit :=
  do 
...
unsafe def for (p : parse parser.pexpr) (occs : parse (list_of small_nat)) (c : itactic) : conv Unit :=
  do 
    let (r, lhs, _) ← tactic.target_lhs_rhs 
    let pat ← tactic.pexpr_to_pattern p 
    let s ← simp_lemmas.mk_default 
...
unsafe def simp (no_dflt : parse only_flag) (hs : parse tactic.simp_arg_list) (attr_names : parse with_ident_list)
  (cfg : tactic.simp_config_ext := {  }) : conv Unit :=
  do 
...
unsafe def guard_lhs (p : parse texpr) : tactic Unit :=
  do 
    let t ← lhs 
...
private unsafe def rw_lhs (h : expr) (cfg : rewrite_cfg) : conv Unit :=
  do 
    let l ← conv.lhs 
...
private unsafe def conv_at (h_name : Name) (c : conv Unit) : tactic Unit :=
  do 
...
private unsafe def conv_target (c : conv Unit) : tactic Unit :=
  do 
    let t ← target 
