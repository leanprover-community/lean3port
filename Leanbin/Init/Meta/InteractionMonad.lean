...
@[inline]
unsafe def interaction_monad_fmap (f : α → β) (t : m α) : m β :=
  fun s => interaction_monad.result.cases_on (t s) (fun a s' => success (f a) s') fun e s' => exception e s'

@[inline]
unsafe def interaction_monad_bind (t₁ : m α) (t₂ : α → m β) : m β :=
  fun s => interaction_monad.result.cases_on (t₁ s) (fun a s' => t₂ a s') fun e s' => exception e s'

@[inline]
unsafe def interaction_monad_return (a : α) : m α :=
  fun s => success a s

unsafe def interaction_monad_orelse {α : Type u} (t₁ t₂ : m α) : m α :=
  fun s =>
    interaction_monad.result.cases_on (t₁ s) success
      fun e₁ ref₁ s' => interaction_monad.result.cases_on (t₂ s) success exception

@[inline]
unsafe def interaction_monad_seq (t₁ : m α) (t₂ : m β) : m β :=
  interaction_monad_bind t₁ fun a => t₂

unsafe instance interaction_monad.monad : Monadₓ m :=
...
unsafe def interaction_monad.silent_fail {α : Type u} : m α :=
  fun s => exception none none s

unsafe def interaction_monad.failed {α : Type u} : m α :=
  interaction_monad.fail "failed"

unsafe def interaction_monad.orelse' {α : Type u} (t₁ t₂ : m α) (use_first_ex := tt) : m α :=
  fun s =>
    interaction_monad.result.cases_on (t₁ s) success
      fun e₁ ref₁ s₁' =>
        interaction_monad.result.cases_on (t₂ s) success
          fun e₂ ref₂ s₂' => if use_first_ex then exception e₁ ref₁ s₁' else exception e₂ ref₂ s₂'
