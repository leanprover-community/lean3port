...
attribute [no_rsimp] id.def Ne.def not_true not_false_iff ne_self_iff_false eq_self_iff_true heq_self_iff_true
  iff_not_selfₓ not_iff_selfₓ true_iff_false false_iff_true And.comm And.assoc And.left_comm and_trueₓ true_andₓ
  and_falseₓ false_andₓ not_and_selfₓ and_not_selfₓ and_selfₓ Or.comm Or.assoc Or.left_comm or_trueₓ true_orₓ or_falseₓ
  false_orₓ or_selfₓ iff_trueₓ true_iffₓ iff_falseₓ false_iffₓ iff_selfₓ implies_true_iff false_implies_iff if_t_t
  if_true if_false
...
| e =>
  if ¬e.is_app then ff else
    let fn := e.get_app_fn 
    if ¬fn.is_constant then ff else
      let nargs := e.get_app_num_args 
      let fname := fn.const_name 
      if fname = `` HasZero.zero ∧ nargs = 2 then tt else
        if fname = `` HasOne.one ∧ nargs = 2 then tt else
          if fname = `` bit0 ∧ nargs = 3 then is_value_like e.app_arg else
            if fname = `` bit1 ∧ nargs = 4 then is_value_like e.app_arg else
              if fname = `` Charₓ.ofNat ∧ nargs = 1 then is_value_like e.app_arg else ff

/-- Return the size of term by considering only explicit arguments. -/
unsafe def explicit_size : expr → tactic Nat
| e =>
  if ¬e.is_app then return 1 else
    if is_value_like e then return 1 else
      fold_explicit_args e 1
        fun n arg =>
          do 
...
unsafe def choose (ccs : cc_state) (e : expr) : tactic expr :=
  do 
    let sz ← explicit_size e 
    let p ←
      ccs.mfold_eqc e (e, sz)$
          fun p e' =>
            if p.2 = 1 then return p else
              do 
                let sz' ← explicit_size e' 
...
unsafe def rsimplify (ccs : cc_state) (e : expr) (m : Option repr_map := none) : tactic (expr × expr) :=
  do 
    let m ←
      match m with 
...
unsafe def collect_implied_eqs (cfg : config := {  }) (extra := hinst_lemmas.mk) : tactic cc_state :=
  do 
    focus1$
        using_smt_with { emAttr := cfg.attr_name }$
          do 
...
unsafe def rsimplify_goal (ccs : cc_state) (m : Option repr_map := none) : tactic Unit :=
  do 
    let t ← target 
...
unsafe def rsimp (cfg : config := {  }) (extra := hinst_lemmas.mk) : tactic Unit :=
  do 
    let ccs ← collect_implied_eqs cfg extra 
    try$ rsimplify_goal ccs

unsafe def rsimp_at (h : expr) (cfg : config := {  }) (extra := hinst_lemmas.mk) : tactic Unit :=
  do 
    let ccs ← collect_implied_eqs cfg extra 
