...
unsafe def get_env : parser environment :=
  fun s => success s.env s
...
unsafe def cur_pos : parser Pos :=
  fun s => success (parser_state.cur_pos s) s
...
unsafe def parser_orelse (p₁ p₂ : parser α) : parser α :=
  fun s =>
    let pos₁ := parser_state.cur_pos s 
    result.cases_on (p₁ s) success
      fun e₁ ref₁ s' =>
        let pos₂ := parser_state.cur_pos s' 
...
unsafe instance cast (p : lean.parser (reflected_value α)) : reflectable (val p) :=
  { full := p }
...
unsafe instance optionalₓ {α : Type} [reflected α] (p : parser α) [r : reflectable p] : reflectable (optionalₓ p) :=
...
unsafe def run_with_input {α} : parser α → Stringₓ → tactic α :=
  fun p s => Prod.fst <$> run (with_input p s)
