...
| ⟨l₁, c₁⟩, ⟨l₂, c₂⟩ =>
  if h₁ : l₁ = l₂ then
    if h₂ : c₁ = c₂ then is_true (Eq.recOnₓ h₁ (Eq.recOnₓ h₂ rfl)) else
      is_false fun contra => Pos.noConfusion contra fun e₁ e₂ => absurd e₂ h₂
...
@[class]
unsafe def reflected {α : Sort u} : α → Type :=
  fun _ => expr
...
unsafe instance : DecidableRel expr.lt_prop :=
  fun a b => Bool.decidableEq _ _
