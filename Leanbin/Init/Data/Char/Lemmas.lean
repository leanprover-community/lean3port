...
theorem val_of_nat_eq_of_is_valid {n : Nat} : IsValidChar n → (of_nat n).val = n := by
  intro h <;> unfold of_nat <;> rw [dif_pos h]

theorem val_of_nat_eq_of_not_is_valid {n : Nat} : ¬IsValidChar n → (of_nat n).val = 0 := by
  intro h <;> unfold of_nat <;> rw [dif_neg h]

theorem of_nat_eq_of_not_is_valid {n : Nat} : ¬IsValidChar n → of_nat n = of_nat 0 := by
