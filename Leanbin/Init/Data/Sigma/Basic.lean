 ...
 (Command.theorem
  ...
  (Command.declValEqns
   (Term.matchAltsWhereDecls
    (Term.matchAlts
     [(Term.matchAlt "|" [(Term.anonymousCtor "⟨" [`x "," `hx] "⟩")] "=>" (Term.anonymousCtor "⟨" [`x "," `hx] "⟩"))])
    []))
  []
  []))
...
theorem ex_of_psig { α : Type u } { p : α → Prop } : Σ' x , p x → ∃ x , p x | ⟨ x , hx ⟩ => ⟨ x , hx ⟩
   ...
   (Term.typeSpec
    ...
    (Term.forall
     ...
     [(Term.implicitBinder
      ...
      (Term.simpleBinder
       [`h₁]
       [(Term.typeSpec ":" («term_=_» (Term.proj `p₁ "." (fieldIdx "1")) "=" (Term.proj `p₂ "." (fieldIdx "1"))))])]
     ","
     (Term.arrow
      («term_=_»
       (Term.paren
        "("
        [(Term.app `Eq.recOnₓ [`h₁ (Term.proj `p₁ "." (fieldIdx "2"))])
         [(Term.typeAscription ":" (Term.app `β [(Term.proj `p₂ "." (fieldIdx "1"))]))]]
        ")")
       "="
  ...
  (Command.declValEqns
   (Term.matchAltsWhereDecls
    (Term.matchAlts
     [(Term.matchAlt
       "|"
       [(Term.anonymousCtor "⟨" [`a "," `b] "⟩") "," (Term.anonymousCtor "⟨" [`a "," `b] "⟩") "," `rfl "," `rfl]
       "=>"
       `rfl)])
    []))
  []
...
[PrettyPrinter.parenthesize] parenthesizing (cont := (none, [anonymous]))
  (Term.app `β [(Term.proj `p₂ "." (fieldIdx "1"))])
[PrettyPrinter.parenthesize.backtrack] unexpected node kind 'Lean.Parser.Term.app', expected 'antiquot'
...
[PrettyPrinter.parenthesize.backtrack] unexpected node kind 'Lean.Parser.Term.typeSpec', expected 'Lean.Parser.Term.typeSpec.antiquot'
[PrettyPrinter.parenthesize] parenthesizing (cont := (none, [anonymous]))
  («term_=_» (Term.proj `p₁ "." (fieldIdx "1")) "=" (Term.proj `p₂ "." (fieldIdx "1")))
[PrettyPrinter.parenthesize.backtrack] unexpected node kind '«term_=_»', expected 'antiquot'
[PrettyPrinter.parenthesize] parenthesizing (cont := (none, [anonymous]))
  (Term.proj `p₂ "." (fieldIdx "1"))
[PrettyPrinter.parenthesize.backtrack] unexpected node kind 'Lean.Parser.Term.proj', expected 'antiquot'
...
protected
  theorem
    Sigma.eq
    : ∀ { p₁ p₂ : Σ a : α , β a } h₁ : p₁ . 1 = p₂ . 1 , ( Eq.recOnₓ h₁ p₁ . 2 : β p₂ . 1 ) = p₂ . 2 → p₁ = p₂
    | ⟨ a , b ⟩ , ⟨ a , b ⟩ , rfl , rfl => rfl
