...
unsafe def get_constructors_for (e : expr) : tactic (List Name) :=
  do 
    let env ← get_env 
...
unsafe def left : tactic (List (Name × expr)) :=
  do 
    let tgt ← target' 
...
unsafe def right : tactic (List (Name × expr)) :=
  do 
    let tgt ← target' 
    let [c₁, c₂] ← get_constructors_for tgt |
      fail "left tactic failed, target is not an inductive datatype with two constructors"
...
unsafe def constructor_idx (idx : Nat) : tactic (List (Name × expr)) :=
  do 
...
unsafe def split : tactic (List (Name × expr)) :=
  do 
...
unsafe def existsi (e : expr) : tactic Unit :=
  do 
    let [c] ← target' >>= get_constructors_for |
